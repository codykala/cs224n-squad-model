...
from modules import *
# from modules import RNNEncoder, SimpleSoftmaxLayer, BasicAttn, Bidaf, BidafModelingLayer, Coattention
...
class QAModel(object):
    ...
    def __choose_optimizer(self):
        """ Returns a op to train the model 

        By default, the Adam optimizer is used to train our learning models.
        From multiple papers, it seems that Adam is somewhat of the "industry
        standard" when it comes to training deep neural networks.

        In "Improving Generalization Performance by Switching from Adam to SGD"
        by Keskar and Socher, it was shown that SGD outperforms Adam at later 
        stages of training, so it might be worthwhile to consider using SGD.

        In "Bi-Directional Attention Flow for Machine Comprehension" by Seo et al,
        the AdaDelta optimizer was used to achieve state-of-the-art results with
        the Bidirectional Attention Flow layer.

        Inputs:
            None

        Outputs:
            opt: an op for the desired optimizer
        """
        # Use Batch Gradient Descent to optimize 
        if self.FLAGS.optimizer == 'gd':
            opt = tf.train.GradientDescentOptimizer(learning_rate=self.FLAGS.learning_rate)
        # Use AdaDelta to optimize
        elif self.FLAGS.optimizer == 'adadelta':
            opt = tf.train.AdadeltaOptimizer(learning_rate=self.FLAGS.learning_rate)
        # Use Adam to optimize (default)
        else:
    ...
    def __pass_through_modeling_layer(self, context_hiddens, question_hiddens):
        """ Adds specified layers to TensorFlow graph and returns blended representation.

        Adds specified layers (using the appropriate flags)  to the TensorFlow 
        graph and creates a blended representation from the outputs of these 
        layers to feed into the neural net.
        
        Inputs:
          context_hiddens: The context hidden states
            Tensor shape (batch_size, context_len, 2 * hidden_size)
          question_hiddens: The question hidden states
            Tensor shape (batch_size, question_len, 2 * hidden_size)
        
        Outputs:
          outputs: The a list of Tensor outputs from each layer in the Tensorflow graph
        """
        outputs = []

        # Use bidrectional attention flow layer model
        if self.FLAGS.model == 'bidaf':
            bidaf_layer = Bidaf(self.keep_prob, 2 * self.FLAGS.hidden_size)
            bidaf_output = bidaf_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask)   # Tensor shape (batch_size, num_context, 8 * hidden_size)
            outputs.append(bidaf_output)
            model_layer = BidafModelingLayer(8 * self.FLAGS.hidden_size, self.FLAGS.hidden_size)
            model_output = model_layer.build_graph(bidaf_output)                                      # Tensor shape (batch_size, num_context, 2 * hidden_size)
            outputs.append(model_output)

        # Use coattention model
        elif self.FLAGS.model == 'coattn':
            coattn_layer = Coattention(self.keep_prob, 2 * self.FLAGS.hidden_size)
            coattn_output = coattn_layer.build_graph(context_hiddens, question_hiddens, self.qn_mask) 
            outputs.append(coattn_output)

        # Use baseline model
        elif self.FLAGS.model == 'baseline':
            basic_layer = BasicAttn(self.keep_prob, 2 * self.FLAGS.hidden_size, 2 * self.FLAGS.hidden_size)
            _, basic_output = basic_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens) # Tensor shape (batch_size, num_context, 2 * hidden_size)
            outputs.append(basic_output)

        else:
            raise Exception("Unexpected value of FLAGS.model: {}".format(self.FLAGS.model))
                       
        return outputs


    def __pass_through_output_layer(self, model_outputs):
        """ Passes the blended representations through an output layer in the graph

        Different models use different output layers.  To use a given output, supply
        the appropriate value for FLAGS.model.

        Inputs:
            model_outputs: A list of Tensor outputs generated by the modeling layer
        Outputs:
            results: a dictionary holding the logits and probability distributions for
                    the start and end locations.  The results dictionary always has
                    the following key-value pairs: 

                    key: "logits_start",     value: Tensor shape (batch_size, context_len)
                    "probdist_start",        value: Tensor shape (batch_size, context_len), rows sum to 1
                    "logits_end",            value: Tensor shape (batch_size, context_len)
                    "probdist_end".          value: Tensor shape (batch_size, context len), rows sum to 1
        """

        # Use bidaf output layer
        if self.FLAGS.model == 'bidaf':
            output_layer = BidafOutputLayer(self.FLAGS.hidden_size)
            results = output_layer.build_graph(model_outputs)

        # Use coattention output layer
        elif self.FLAGS.model == 'coattn':
            raise Exception("Output layer for coattention not yet implemented.")

        # Use baseline output layer
        elif self.FLAGS.model == 'baseline':
            output_layer = BaselineOutputLayer(self.FLAGS.hidden_size)
            results = output_layer.build_graph(model_outputs, self.context_mask)

        else:
            raise Exception("Unexpected value of FLAGS.output: {}".format(self.FLAGS.output))    
    ...
    def build_graph(self):
        ...
        # Use some combination of attention layers and modeling layers to obtain
        # a blended representation to feed to the output layer
        model_outputs = self.__pass_through_modeling_layer(context_hiddens, question_hiddens)
        results = self.__pass_through_output_layer(model_outputs)

        # Assign the logits and distribution calculations accordingly
        self.logits_start = results["logits_start"]
        self.probdist_start = results["probdist_start"]
        self.logits_end = results["logits_end"]
        self.probdist_end = results["probdist_end"]
    ...
    def add_loss(self):
        ...
        Outputs:
            self.loss_start: Loss for the predicted start position of the answer span
                Tensor scalar
            self.loss_end: Loss for the predicted end position of the answer span
                Tensor scalar
            self.loss: Total loss
                Tensor scalar

        IMPORTANT: 
            self.ans_span: Contains the gold start and end locations
